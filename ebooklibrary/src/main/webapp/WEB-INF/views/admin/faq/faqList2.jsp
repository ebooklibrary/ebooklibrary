					...
					<c:forEach var="vo" items="${faqList}">
						...
						<li id="FAQ_List_${i }" name="TrFAQ_Content" class="off">
							<dl class="faqDl">
								...
								<dd><a href="<c:url value='/library/serviceCenter/faq/faqEdit.do?faqNo=${vo.faqNo}'/>"  >수정하기</a> </dd>
								<dd>
									<form name="frmAddCount" id="readCount" method="post" action="<c:url value='/library/serviceCenter/faq/faqfixed.do'/>" >
			   							<input type="hidden" name="faqNo" value="${vo.faqNo }"/>
										<input type="submit" value="고정글"/>
										<label>
										
										<c:if test="${vo.fixed=='Y' }">
											고정글
										</c:if>
										<c:if test="${vo.fixed=='N' }">
											비고정글
										</c:if>
										</label>
									</form>
