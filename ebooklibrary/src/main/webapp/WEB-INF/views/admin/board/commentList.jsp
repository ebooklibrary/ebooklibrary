			...
			<c:forEach var="clist" items="${ commentList}">			
				<tr class="cmtTr_1">
					<td class="cmt_td1" rowspan="2">
							<c:if test="${param.complete!='Y' }">
								<c:if test="${sessionScope.adminMemberNo==param.memberNo &&  param.memberNo!=clist.memberNo}">
									<form id="SelCmt" method="post" action="<c:url value='/admin/completeQna.do'/>">
					...
					<td>
						<c:if test="${sessionScope.adminMemberNo==clist.memberNo}">
						</c:if>
					</td>
				</tr>
				<tr >
					<td class="cmt_td4">
						<c:if test="${!empty sessionScope.adminMemberNo }">
							<c:if test="${sessionScope.adminMemberNo!=clist.memberNo}">
								<input type="button" onclick="reVisi(${i})" value="덧글쓰기" class="bt_cmtList"/>
							</c:if>			
						</c:if>
						<c:if test="${sessionScope.adminMemberNo==clist.memberNo}">
							<form id="delCmtfrm" method="post" action="<c:url value='/admin/commentDelete.do'/>">
				...
				<tr>	
					<td class="recmt" id="${i}" name="${i}" colspan="5" style="display:none;" >
						<form id="refrm" method="post" action="<c:url value='/admin/commentWrite.do'/>">
							...
							<input type="hidden" name="userName" value="${sessionScope.adminUserId}"/>
							<input type="hidden" name="memberNo" value="${sessionScope.adminMemberNo}"/>
							<label class="replyLb">${sessionScope.adminUserId} </label> 
				...
				<tr>
					<td class="cmt_td5" id="${j}" name="${j}"  colspan="5" style="display:none;" >
						<form id="refrm" method="post" action="<c:url value='/admin/commentEdit.do'/>">
							<input type="hidden" name="groupNo" value="${clist.groupNo }"/>
							<input type="hidden" name="commentNo" value="${clist.commentNo }"/>
...
<div id="divCmw" style="visibility:visible;"/>	
	<form name="cmWrite" id="cmWrite" method="post"
		action="<c:url value='/admin/commentWrite.do'/>" >
		<input type="hidden" name="qnaNo" value="${param.qnaNo }"/>
		<input type="hidden" name="userName" value="${sessionScope.adminUserId}"/>
		<input type="hidden" name="memberNo" value="${sessionScope.adminMemberNo}"/>
		<label>${sessionScope.adminUserId} </label> 
